...
int indice = 0, position = 0, contador = 0, k = -1, y = -1, positionTlb = 0, contadorTlb = 0, positionNew = 0, stop, confirmou;
    ...
    while (fscanf(arq, "%d", &numInstrucao) != EOF)
    {
        ...
        for (int j = 0; j < 16; j++)
        {
            ...
            if (tamTlb[j].tlbPage == pageDecimal)
            {
                if(tamTlb[j].tlbBit = 1){
                    stop = 1;
                    tlbHits = tlbHits + 1;
                    printf("---TLB HIT ATUAL = %.0f---\n", tlbHits);
                    tamPageTable[pageDecimal].time = contador;
                    tamTlb[j].tlbTime = contadorTlb;
                    printf("---CONTADOR TLB ATUAL 1: %d---", contadorTlb);
        ...
        else if (tamPageTable[pageDecimal].bit == 1)
        {
            ...
            {  
                ...
                tamTlb[positionTlb].tlbTime = contadorTlb;
                printf("---INDICE DA TLB: %d---", positionTlb);
                printf("---INDICE DA MEMORIA: %d---", indice);
                printf("---CONTADOR TLB ATUAL 2_3: %d---", contadorTlb);
            }
            ...
            if (positionTlb >= 16)
            {
                ...
                {   
                    ...
                    if (confirmou == 1)
                    {
                        printf("---TEMPO MENOR TLB: %d---", menorTlb);
                        printf("---VALOR DE Y: %d---", y);
                        // ATUALIZAR NOVO
                        tamTlb[y].tlbBit = 1;
                        tamTlb[y].tlbFrame = indice;
                        tamTlb[y].tlbPage = pageDecimal;
                        printf("---INDICE ATUAL: %d---", tamTlb[y].tlbFrame);
                        printf("---PAGE DECIMAL ATUAL: %d---", tamTlb[y].tlbPage);
                        // TEMPO GLOBAL
                        tamTlb[y].tlbTime = contadorTlb;
                        printf("---CONTADOR TLB ATUAL 4: %d---", tamTlb[y].tlbTime);
                    }
                }
            }
        }
...
{
    arq = fopen(argv1, "r"); // argv1 = addresses.txt
    int quantidadeFormato = strlen(argv1); // quantidadeFormato = 13
    separarFormato(argv1, formato, quantidadeFormato); // formato = txt
