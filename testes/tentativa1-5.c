...
int tlbFuction(int argc, char *argv[]);
...
int indice = 0, position = 0, contador = 0, k = -1, positionTlb = 0, contadorTlb = 0, positionNew = 0;
...
int main(int argc, char *argv[])
{
    ...
    for (i = 0; i < 1000; i++)
    {
        ...
        // O PAGE NAO ESTÁ NA TABELA DE PAGINAS, NÃO ESTÁ NA TLB E DEU PAGE FAULT
        ...
        {
            tamPageTable[pageDecimal].time = contador;
            // NÃO ESTÁ NA TLB E DEU TLB MISS
            for (int j = 0; j < 16; j++)
            {
...
int tlbFuction(int argc, char *argv[])
{
    ...
    if (positionTlb == 16)
    {
        if (!strcmp(argv[3], "fifo"))
        {
            ...
            for (int j = 0; j < 16; j++)
            {
                if (tamTlb[j].tlbFrame == positionNew)
                {
                    tamTlb[j].tlbBit = 0;
                    tamTlb[j].tlbFrame = -1;
                    tamTlb[j].tlbPage = -1;
            ...
            tamTlb[positionNew].tlbBit = 1;
            tamTlb[positionNew].tlbPage = pageDecimal;
            tamTlb[positionNew].tlbFrame = tamPageTable[pageDecimal].frame;
            positionNew = positionNew + 1;
        }
    }
