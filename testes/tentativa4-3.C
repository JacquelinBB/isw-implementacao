    ...
    for (i = 0; i < 1000; i++)
    {
        ...
        if (tamPageTable[pageDecimal].bit == 0)
        {
            ...
            else
            {
                ...
                // tamPageTable[pageDecimal].frame = index; //VALUE FICA ERRADO
                lerBackStore(pageDecimal);
                index = index + 1;
                printf("--POS INDEX--");
            }
            printf("---Check indice PF: %d ---", tamPageTable[pageDecimal].frame);
            printf("---INDICE IN TRAVADO: %d ---", indice);
            printf("---INDEX IN: %d ---", index);
        }
        ...
        if (tamPageTable[pageDecimal].bit == 1)
        {
            ...
            printf("---Check indice PF2: %d ---", tamPageTable[pageDecimal].frame);
